...
def ticket_system(db_connection):
    with patch('sales_system.interface.open'):
        yield TicketSystem(db_connection, Mock())
...
def test_InterfaceKeywords_PassedCorrectly(db_connection, ticket_system, monkeypatch,
    ...
    function_mock.assert_called_once_with(db_connection, *([ANY]*additional_argc))
